      ...
      <template v-for="item in listData">
        <el-form-item :label="item.label" :prop="item.prop" :key="item.prop" :label-width="getLabelWidth(item.width)" v-if="!item.showIf || (item.showIf && item.valIf === searchForm[item.propIf] || (item.valOr && item.valOr === searchForm[item.propIf]))">
          <div class="item-class flex" :class="{'input-change':item.max && item.max>0}" :style="{ width: item.itemWidth || '100%' }">
            <!-- input -->
            <el-input v-if="!item.type || item.type == 'text' || item.type == 'textarea'" v-model="searchForm[item.prop]" :maxlength="item.max || -1" :show-word-limit="item.max > 0" :type="item.type || 'text'" :placeholder="showPlaceholder ? item.placeholder || '请输入' : ''"></el-input>
            <!-- number -->
            <el-input v-if="item.type == 'number'" v-model.number="searchForm[item.prop]" :maxlength="item.max || -1" :show-word-limit="item.max > 0" :type="item.type || 'text'" :placeholder="showPlaceholder ? item.placeholder || '请输入':''"></el-input>
            <!-- text -->
            ...
            <!-- select -->
            <el-select class="w100" v-if="item.type == 'select'" :filterable="item.filterable" :multiple="item.multiple" :remote="item.remote" v-model="searchForm[item.prop]" :placeholder="showPlaceholder ? item.placeholder || '请选择':''" :disabled="item.disabled" :remote-method="e=>remoteMethod(e,item.prop)" :loading="remoteLoading" @change="e=>selectChange(e,item.renderProp || item.prop)" :allow-create="item.allowCreate" :default-first-option="item.defaultFirstOption">
